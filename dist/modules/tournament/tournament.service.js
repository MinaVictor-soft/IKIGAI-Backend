...
class TournamentService {
    async createTournament(input) {
        ...
        if (selectedTeamIds.length !== expectedTeamCount) {
            throw new Error(`Expected ${expectedTeamCount} teams (${numberOfGroups} groups × ${teamsPerGroup} per group), ` +
        ...
        for (const group of groups) {
            const createdGroup = await database_1.default.tournamentGroup.create({
                data: {
        ...
        const updatedTournament = await database_1.default.tournament.update({
    ...
    async notifyTournamentCreated(tournament) {
        try {
            // Get all users
            ...
            await notifications_service_1.notificationsService.createBulkNotifications(userIds, 'TOURNAMENT_CREATED', '🏆 New Tournament', `A new tournament "${tournamentTitle}" has been created with ${tournament.numberOfGroups} groups!`, {
                tournamentId: tournament.id,
            ...
            const allPushTokens = await push_notifications_service_1.pushNotificationsService.getAllActivePushTokens();
            if (allPushTokens.length > 0) {
                await push_notifications_service_1.pushNotificationsService.sendPushNotification({
    ...
    async generateGroupMatches(tournamentId) {
        const groups = await database_1.default.tournamentGroup.findMany({
            where: { tournamentId },
            include: {
                teams: { include: { team: true } },
            },
        ...
        for (const group of groups) {
            ...
            for (let i = 0; i < teamIds.length; i++) {
                for (let j = i + 1; j < teamIds.length; j++) {
                    await database_1.default.tournamentMatch.create({
                        data: {
                            tournamentId,
                            stage: 'GROUP_STAGE',
                            team1Id: teamIds[i],
    ...
    async getTournament(id) {
        return database_1.default.tournament.findUnique({
            where: { id },
            include: {
                groups: {
                    include: {
    ...
    async listTournaments(status) {
        return database_1.default.tournament.findMany({
    ...
    async completeMatch(tournamentId, matchId, team1Goals, team2Goals) {
        const match = await database_1.default.tournamentMatch.findUnique({
            where: { id: matchId },
            include: {
                tournament: true,
                team1: true,
        ...
        const updatedMatch = await database_1.default.$transaction(async (tx) => {
            // Update match
    ...
    async notifyMatchResultApplied(tournamentId, match) {
        try {
            ...
            await notifications_service_1.notificationsService.createBulkNotifications(userIds, 'MATCH_RESULT_APPLIED', '⚽ Match Result', `${team1Name} ${match.team1Goals} - ${match.team2Goals} ${team2Name}`, {
                tournamentId,
                matchId: match.id,
    ...
    async checkAndNotifyTournamentComplete(tournamentId) {
        try {
            const tournament = await database_1.default.tournament.findUnique({
                where: { id: tournamentId },
                include: {
    ...
    async notifyTournamentComplete(tournament) {
        try {
            const users = await database_1.default.user.findMany({
                where: { deletedAt: null },
                select: { id: true },
            ...
            // Create database notifications
            await notifications_service_1.notificationsService.createBulkNotifications(userIds, 'TOURNAMENT_COMPLETED', '🏆 Tournament Complete', `${tournamentName} has ended! Champion: ${championName}`, {
                tournamentId: tournament.id,
    ...
    async notifyMatchCreated(tournamentId, match) {
        try {
            const users = await database_1.default.user.findMany({
                where: { deletedAt: null },
                select: { id: true },
            ...
            const team2Name = match.team2?.name || 'Team 2';
            // Create database notifications
            await notifications_service_1.notificationsService.createBulkNotifications(userIds, 'MATCH_CREATED', '⚽ New Match Scheduled', `${team1Name} vs ${team2Name} - Get ready!`, {
                tournamentId,
                matchId: match.id,
    ...
    async advanceToKnockout(tournamentId) {
        const tournament = await database_1.default.tournament.findUnique({
            where: { id: tournamentId },
            include: {
                groups: {
    ...
    calculateKnockoutStages(teamCount) {
        const stages = [];
    ...
    async generateKnockoutMatches(tournamentId, qualifiedTeams, stage) {
        ...
        for (const match of matches) {
            const createdMatch = await database_1.default.tournamentMatch.create({
                data: {
                    tournamentId,
                    stage: stage,
                    team1Id: match.team1Id,
    ...
    async generateNextKnockoutRound(tournamentId) {
        const tournament = await database_1.default.tournament.findUnique({
            where: { id: tournamentId },
            include: {
                tournamentMatches: true,
    ...
    async getGroupStandings(tournamentId, groupId) {
        return database_1.default.tournamentTeam.findMany({
            where: {
                tournamentId,
                groupId,
            },
    ...
    async updateMatch(tournamentId, matchId, updates) {
        const match = await database_1.default.tournamentMatch.findUnique({
            where: { id: matchId },
        ...
        return database_1.default.tournamentMatch.update({
            where: { id: matchId },
            data: {
    ...
    async getTournamentBracket(tournamentId) {
        const matches = await database_1.default.tournamentMatch.findMany({
            where: { tournamentId },
            include: {
                team1: true,
                team2: true,
